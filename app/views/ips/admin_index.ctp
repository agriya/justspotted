...
?>
	<tr<?php echo $class;?>>
       <td><?php echo $this->Form->input('Ip.'.$ip['Ip']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$ip['Ip']['id'], 'label' => false, 'class' => $status_class.' js-checkbox-list')); ?></td>
		...
		<td class="dl">
		<?php if(!empty($ip['Ip']['ip'])): ?>
                            <?php echo  $this->Html->cText($ip['Ip']['ip']);
							?>
							<p>
							<?php
                            if(!empty($ip['Country']['name'])):
                                ?>
                                <span class="flags flag-<?php echo strtolower($ip['Country']['iso2']); ?>" title ="<?php echo $ip['Country']['name']; ?>">
									<?php echo $ip['Country']['name']; ?>
								</span>
                                <?php
                            endif;
							 if(!empty($ip['City'])):
                            ?>
                            <span> 	<?php echo $ip['City']['name']; ?>    </span>
                            <?php endif; ?>
                            </p>
                        <?php else: ?>
							<?php echo __l('N/A'); ?>
