...
<div>    
    ...
    <?php echo $this->Html->link(__l('Trigger Cron Manually'), array(('controller' => 'crons', 'action' => 'main', '?f=' . $this->request->url, 'admin' => false), array('class' => 'update-status', 'title' => __l('Trigger cron functions manually (If for some reasons, cron is not getting triggered, clicking this link will trigger the functionality)')));?>
    </div>
</div>