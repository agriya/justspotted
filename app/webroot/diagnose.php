...
$facebook_settings_id = 19;
$twitter_settings_id = 20;
$foursquare_settings_id= 45;
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<title>JustSpotted - Diagnostic Tool</title>
<style type="text/css">
body{
	color:#4c4c4c;
	line-height:18px;
...
$master_db = _db_check('master',$facebook_settings_id,$twitter_settings_id,$foursquare_settings_id);
$default_db = _db_check('default',$facebook_settings_id,$twitter_settings_id,$foursquare_settings_id);
...
<?php
//---------------- Default table -------------------------------------------------------
function _db_check($database = 'default',$facebook_settings_id,$twitter_settings_id,$foursquare_settings_id)
{
    ...
    if ($link) {
        ...
        if ($db_selected) {
           ...
           $result='';		
            ...
            $result = mysql_query("select * from settings where setting_category_parent_id in (1,3,6,37)", $link);
            $db_info['payment_gateway']['error'] = 'None';
            if (!$result) {
                echo $db_info['payment_gateway']['error'] = mysql_error();
            } else {
                $db_info['settings'][$twitter_settings_id] = $db_info['settings'][$facebook_settings_id] = $db_info['settings'][$foursquare_settings_id] = array(
                    'Mandatory' => array() ,
                    'Credentials' => array() ,
                    'Not Mandatory' => array()
                );
                $mandatory = array(
					...
					'foursquare.consumer_key',
					'foursquare.consumer_secret',
					'foursquare.site_foursquare_venue_id',
					'foursquare.site_user_access_token',
					'foursquare.site_user_fs_id'
                );
                $site_settings = array(   
                ...
                while ($row = mysql_fetch_assoc($result)) {
                    ...
                    } elseif ($row['setting_category_id'] == $facebook_settings_id || $row['setting_category_id'] == $twitter_settings_id || $row['setting_category_id'] == $foursquare_settings_id) {
                        $db_info['settings'][$row['setting_category_id']]['Mandatory'][] = array(
							'name' => $row['name'],
                            'label' => $row['label'],
                            'value' => $row['value']
					...
					} elseif (in_array($row['name'], $iphone_settings)) {
                        $db_info['settings']['iphone']['Mandatory'][] = array(
							'name' => $row['name'],
                            'label' => $row['label'],
                            'value' => $row['value']
                        );
				...
				sort($db_info['settings'][$foursquare_settings_id]['Mandatory']);
            }
...
if (isset($_GET['action']) && $_GET['action'] == 'post' && $_GET['to'] != '' && $_GET['to'] == 'facebook') {
    if (!empty($master_db['settings'][$facebook_settings_id]['Mandatory'][0]['value']) && !empty($master_db['settings'][$facebook_settings_id]['Mandatory'][2]['value']) && !empty($master_db['settings'][$facebook_settings_id]['Mandatory'][3]['value']) && !empty($master_db['settings'][$facebook_settings_id]['Mandatory'][4]['value'])) {
        ...
        $fb_page_id = $master_db['settings'][$facebook_settings_id]['Mandatory'][4]['value'];
...
////////////Post foursquare//////////////////////
$foursqr_check = '';
if (isset($_GET['action']) && $_GET['action'] == 'post' && $_GET['to'] != '' && $_GET['to'] == 'foursquare') {
    if (!empty($master_db['settings'][$foursquare_settings_id]['Mandatory'][0]['value']) && !empty($master_db['settings'][$foursquare_settings_id]['Mandatory'][1]['value']) && !empty($master_db['settings'][$foursquare_settings_id]['Mandatory'][2]['value']) && !empty($master_db['settings'][$foursquare_settings_id]['Mandatory'][3]['value'])) {
        
		$client_key = $master_db['settings'][$foursquare_settings_id]['Mandatory'][0]['value'];
		$client_secret = $master_db['settings'][$foursquare_settings_id]['Mandatory'][1]['value'];
		$token = $master_db['settings'][$foursquare_settings_id]['Mandatory'][3]['value'];
		include_once '../vendors/foursquare/FoursquareAPI.class.php';
		// Load the Foursquare API library
		$foursquare = new FoursquareAPI($client_key, $client_secret);
		$foursquare->SetAccessToken($token);
		
		$site = $master_db['settings']['site'][0];
		$fs_message = 'This is the test post from ' . $site . ' - Foursquare on ' . date('h:i:s A', time());
		$url = "http://" . $_SERVER['SERVER_NAME'];
		$foursquare_venue_id = $master_db['settings'][$foursquare_settings_id]['Mandatory'][2]['value'];
		$params['text'] = $fs_message;
		$params['url'] = $url;
		$params['venueId'] = $foursquare_venue_id;
		$result = $foursquare->postTips($params);
		$res = json_decode($result);
        if ($res->meta->code == 200) {
            $foursqr_check = 'Successfully posted on Foursquare';
        } else {
            $foursqr_check = 'Post on Foursquare error - ' . $res->meta->code;
        }
    } else {
        $foursqr_check = 'Foursquare API values should not be empty';
    }
}
////////////Post foursquare ends//////////////////////
////////////Mail work status//////////////////////
$email_check = '';
...
<?php
if (!empty($foursqr_check)) { ?><center><h2><?php
    echo $foursqr_check . '<br><a href="' . $_SERVER['PHP_SELF'] . '#tweet">Back</a>'; ?></h2></center><?php
} ?>
<?php
...
<h1>JustSpotted Diagnostic Tool</h1>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=justspotted-install#setting_up_files" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=justspotted-install#setting_up_files</a>
	</span>
</p>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=justspotted-install#setting_up_database" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=justspotted-install#setting_up_database</a>
	</span>
</p>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=justspotted-install#manage_settings" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=justspotted-install#manage_settings</a>
	</span>
</p>
<?php
...
?>
            ...
            switch ($key) {
                ...
                case $twitter_settings_id: // Twitter
                    ...
                    break;
				case $foursquare_settings_id: // Foursquare
                    echo 'Foursquare - <a href="' . $_SERVER['PHP_SELF'] . '?action=post&amp;to=foursquare" id="foursqr">Test post</a> to Foursquare';
					//echo 'Foursquare - <a href="#" id="foursqr">Test post</a> to Foursquare';
                    break;
                case 'msn': // Twitter
                    echo 'Invite MSN';
                    break;
...
?>
	...
	<?php
		...
		{
			$setting['Mandatory'][7] = $setting['Mandatory'][0];
			$setting['Mandatory'][8] = $setting['Mandatory'][4];
            unset($setting['Mandatory'][0]);
			unset($setting['Mandatory'][4]);
		}	
		if($key == $twitter_settings_id) //To display twitter credentials in the last rows
		{
			$setting['Mandatory'][7] = $setting['Mandatory'][2];
			$setting['Mandatory'][8] = $setting['Mandatory'][3];
            unset($setting['Mandatory'][2]);
			unset($setting['Mandatory'][3]);
		}	

		if($key == $foursquare_settings_id) //To display foursquare credentials in the last rows
		{
			$setting['Mandatory'][7] = $setting['Mandatory'][3];
			$setting['Mandatory'][8] = $setting['Mandatory'][4];
            unset($setting['Mandatory'][3]);
			unset($setting['Mandatory'][4]);
		}		
		foreach($setting as $data => $step) {
                $i = 0;
				if($key == $facebook_settings_id || $key == $twitter_settings_id || $key == $foursquare_settings_id)
				{
					$count = count($step)+1;
				} else {
					$count = count($step);
...
?>
						<tr>
                            ...
                            } elseif ($mantry['name'] == 'EmailTemplate.admin_email') {
...
<?php							
						...
						if($mantry['name'] == 'foursquare.site_user_access_token')
						{
?>
							<tr>
							<td colspan="2">
                            <div class="cred_detail">
                            The following 2 settings will automatically be updated when clicking "Update Foursquare Credentials" link in Foursquare tab found under admin settings.
                            </div>
                            </td>
                            </tr>
<?php							
