        ...
        autogeocomplete: function(options){
            ...
            this.autocomplete({
                            ...
                            if (item_count < 5) {
								...
								if(components.length){									
									for(var j=0; j<components.length; j++){
										if(components[j].types[0]=="route" || components[j].types[0]=="street_number"){
											if(street){
										...
										if(components[j].types[0]=="locality" || components[j].types[0]=="administrative_area_level_3"){
											if(!city){
								...
								for(i=0; i<address_list.length; i++){	
									switch(address_list[i]){
										...
										case 'STATE':
													...
													break;
										case 'COUNTRY':
													temp = temp.replace("##COUNTRY##", country_lang);
													break;
										case 'ZIPCODE':
													temp = temp.replace("##ZIPCODE##", postal_code);
													break;
                                ...
                                return {
                                    label:  place,
                                    value: label_val, //item.formatted_address,
                                    desc: place_details,
                                    area: area,
                                    city: city,
                                    state: state,
                ...
                select: function(event, ui) {
					...
					if ($(this).attr('id') == 'PropertyAddressSearch' || $(this).attr('id') == 'CompanyAddressSearch') {
						if (ui.item.area == '' || ui.item.city == '' || ui.item.country == '' || ui.item.latitude == '' || ui.item.longitude == '') {
