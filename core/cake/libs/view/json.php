...
// Hack: Temporary fix for the autocomplete jquery lib issue (it's flaky with JSON)
        if ($action=='autocomplete' || $this->action == 'autocomplete') {
			if(isset($this->viewVars['json']['User']) || isset($this->viewVars['json']['Item']) || isset($this->viewVars['json']['Place'])){
				$out = '[';
				$j = 1;			
				foreach($this->viewVars['json'] as $models => $results) {
					$i=1;
					if($i == 1 && $models == 'Place' && (!isset($this->viewVars['json']['User']) && !isset($this->viewVars['json']['Item']) && isset($this->viewVars['json']['Place']))) {
						$out .= '{ "id": "0", "label": "New Place", "value": "New Place", "address": "New Place" , "models": "'.$models.'" }';
					}
					if( ($out != '[') && !empty($results) ){
						$out .=',';
					}
					foreach($results as $val => $text) {
						if($models != 'Place'){
							 $out .= '{ "id": "'.$val.'", "label": "'.$text.'", "value": "'.$text.'" , "models": "'.$models.'" }';
						}
						else{	
							 $out .= '{ "id": "'.$text['Place']['id'].'", "label": "'.$text['Place']['name'].'", "value": "'.$text['Place']['name'].'", "address": "'.$text['Place']['address2'].'" , "models": "'.$models.'" }';
						}	 
						if(count($results) != $i){
							$out .=',';
						}
					...
					/*if(count($this->viewVars['json']) != $j && !empty($results)){
						$out .=',';
					}
					$j++;*/
				}
				...
				foreach($this->viewVars['json'] as $val => $text) {
					 $out .= '{ "id": "'.$val.'", "label": "'.$text.'", "value": "'.$text.'" }';
